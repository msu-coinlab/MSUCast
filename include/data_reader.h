 ...
 *
 * This class reads the different required data by the CBWOP
 */
class DataReader {
public:
    /**
     * @brief Constructor reads an environment variable to set up csvs_path
     */
    DataReader();

    /**
     * @brief Constructor assigns the parameter input to csvs_path
     *
    ...
    /**
     * @brief Reads a CSV file containing the load source to load group information
     */
    void read_load_src_to_bmp_list();

    /**
     * @brief Get the dictionary containing the load source to load source group
    ...
    /**
     * @brief Get the list containing the load source to load source group
     * @param load_src The load source identifier.
     * @return A vector<int> containing the load source group
     */
    const std::vector<int>& get_load_src_to_bmp_list(int load_src) const;

    /**
     * @brief Get the dictionary containing the load source to load source group
     * @return A unordered_map<std::string, int> with the map load source -> load source group
     */
    const std::unordered_map<std::string, int>& get_lc_bmp_from_to() const;

    /**
     * @brief Get the load source to load source group mapping
     * @param lc_bmp_to The load source identifier.
    ...
    /**
     * @brief Provides the load source group by a given load source. If load source is not
     * in the dictionary it returns -1
     *
     * @param load_source The load source identifier.
     * @return The load source group if it is in the dictionary otherwise returns -1
     */
    int get_u_u_group(int load_source) const;

    /**
     * @brief Reads a CSV file containing the lrseg to geography and state information.
     */
    ...
    /**
     * @brief Provides the geography of a given lrseg. If lrseg is not
     * in the dictionary it returns -1
     *
     * @param lrseg The lrseg identifier.
    ...
    /**
     * @brief Provides the state of a given lrseg. If lrseg is not
     * in the dictionary it returns -1
     *
     * @param lrseg The lrseg identifier.
    ...
    /**
     * @brief Reads a CSV file containing the cost per unit for every land BMP
     * @param filename The name of the CSV file.
     * @return Integer status of the reading operation.
     */
    ...
    /**
     * @brief Get the dictionary containing the cost for all land BMPs.
    ...
    /**
     * @brief Provides the cost per unit for a given bmp in a given profile. If bmp is not
     * in the dictionary it returns -1
     *
     * @param profile The profile cost identifier.
     * @param bmp The BMP identifier.

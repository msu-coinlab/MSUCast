...
int selected_pop_size;  /**< Size of population S_t */
int final_pop_size;     /**< Final population size */
int nholes;             /**< Number of reference points having no population member associated with them (i.e., holes) */
int *rank;              /**< Non-domination rank of each population member */
int *selected_pop_indices; /**< Indices of population members up to last front */
int *final_pop_indices;   /**< Indices of population members in array "selected_pop_indices" which are surely included in P_{t+1} */
int *nclstr;            /**< Number of population members associated with each reference point including last front (i.e., "niche count") */
int *nclstr_sure;       /**< Number of population members associated with each reference point that are to be surely selected */
int *idx;               /**< Contains the index of reference point associated with each population point */
int **clstr_info;       /**< clstr_info[i][j] contains indices of population members belonging to jth reference point */
int **clstr_info_limiting_front; /**< clstr_info_limiting_front[i][j] contains indices of population members belonging to last front and to jth reference point */
int *within_clstr_rank; /**< Rank of members within their cluster */
int *hole;              /**< Indices of reference points with 0 niche count */
double *intercept;      /**< Intercept values for calculations */
double **trans_pop;     /**< Transformed population */
double **D;             /**< Perpendicular distance of each population member from each reference point */
int incr_exhausted;     /**< Indicator for exhaustion of increments */

double extreme[2];      /**< Extreme values for population */
bool first = false;     /**< Flag for first execution */
int extreme_idx;        /**< Index of extreme value */

/**
 * @brief Routine to determine the Last Front
 * 
...
/**
 * @brief Routine to form population S_t and P_{t+1} for an unconstrained problem
 */
...
/**
 * @brief Routine to form population S_t and P_{t+1} for a constrained problem
 * 
 * @param pop Pointer to the population structure
 */
...
/**
 * @brief Translates the population based on the ideal point
 * 
 * @param pop Pointer to the population structure
 */
...
/**
 * @brief Finds intercepts for the population
 */
...
/**
 * @brief Associates each population member with the closest reference point
 */
void associate()
{
    int i, j, k;
...
/**
 * @brief Routine to identify the closest population member in a cluster and give it a rank of one while assigning ranks randomly to the remaining members
 * 
 * @param pop Pointer to the population structure
 */
...
/**
 * @brief Removes reference points that have no associated population members
 */
void remove_points()
{
    int i, j, k, t, n, flag;
...
/**
 * @brief Defines the final population based on selection criteria
 */
void define_final_pop()
{
    ...
    nothole = (int *)malloc(nref * sizeof(int));      /* indices of reference points having at least one population member */
...
/**
 * @brief Creates points around a given cluster ID
 * 
 * @param clstrid Cluster ID
 */
...
/**
 * @brief Creates points around a given cluster ID with a different approach
 * 
 * @param clstrid Cluster ID
 */
...
/**
 * @brief Checks for the need to increment reference points
 */
...
/**
 * @brief Main function for elitist sorting of populations
 * 
 * @param mixed_pop Pointer to the mixed population
 * @param new_pop Pointer to the new population
 */
void elitist_sorting(population *mixed_pop, population *new_pop)
{
    ...
    if (curr_gen > 1)
    {
        if (remaining > 0)
        {
            ...
            {
                copy_ind(
                    &mixed_pop->ind[selected_pop_indices[final_pop_indices[i]]],
                    &new_pop->ind[i]);
            }
        }
        else
